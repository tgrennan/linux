...
/* uio_device was exported from 3.18 on. */
...
static unsigned int uio_poll(struct file *filep, poll_table *wait)
{
	...
	if (listener->event_count != event_count) {
		listener->event_count = event_count; /* platina changed: user space need not read to ack interrupt. */
